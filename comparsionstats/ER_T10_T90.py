...
N = 1000
TARGET_MEAN_DEGREE = 6.0
P_ER = TARGET_MEAN_DEGREE / (N - 1)     # ER mean degree ~ p*(N-1)
N_RUNS = 2000
...
def generate_er_graph(n, p, rng):
    """
    Generate an undirected ER G(n,p) graph.

    Returns:
        GraphData with:
        - edges_u, edges_v: arrays of edge endpoints
        - neighbors[node]: list of neighbouring nodes
        - edge_ids[node]: list of edge indices incident on node
    """
    edges_u = []
    edges_v = []

    for i in range(n):
        # sample upper triangle only: j > i
...
def simulate_dynamic_si_fenwick(graph, beta, mu, t_grid, rng, thresholds):
    """
    Dynamic SI on an ER graph with edge flickering.
...
def dynamic_worker(args):
    ...
    graph = generate_er_graph(N, P_ER, rng_graph)
...
def main():
    ...
    # Dynamic ensemble
    # --------------------------------------------------------
    print("Running dynamic ER ensemble...")
    ...
    # --------------------------------------------------------
    print("\nRunning static ER ensemble with beta=1...")
    ...
    plt.title("ER network: average infection curves")
    ...
    plt.title("Effective beta search on ER networks")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    ...
    for thr in THRESHOLDS:
        ...
        plt.xlabel(label)
        plt.ylabel("Density")
        plt.title(f"ER network: {label} distributions")
        plt.legend()
        plt.tight_layout()
        plt.show()
