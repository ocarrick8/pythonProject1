...
BETA_GRID = np.linspace(0.4, 0.5, 21)
...
def update_threshold_times(infected_fraction_prev, infected_fraction_new, t_new, t50, t90):
    """
    Record T50/T90 the first time each threshold is crossed.
    We assign the crossing to the event time t_new.
    """
    if t50 is None and infected_fraction_prev < 0.50 <= infected_fraction_new:
        t50 = t_new
    if t90 is None and infected_fraction_prev < 0.90 <= infected_fraction_new:
        t90 = t_new
    return t50, t90
...
def simulate_dynamic_si_fenwick(graph, beta, mu, t_grid, rng):
    """
    ...
    while t < t_grid[-1] and infected_count < n:
        ...
        if rng.random() < (lambda_inf / lambda_tot):
            ...
            t50, t90 = update_threshold_times(prev_I, new_I, t, t50, t90)
...
def simulate_static_si_fenwick(graph, beta, t_grid, rng):
    """
...
def dynamic_worker(args):
    ...
    curve, t50, t90 = simulate_dynamic_si_fenwick(
        graph=graph,
        beta=BETA_DYNAMIC,
        mu=MU,
        t_grid=T_GRID,
...
def static_worker(args):
    ...
    graph = generate_ba_graph(N, M_ATTACH, rng_graph)
    curve, t50, t90 = simulate_static_si_fenwick(
        graph=graph,
        beta=beta_static,
        t_grid=T_GRID,
        rng=rng_sim
...
def run_ensemble(worker_fn, arglist, n_jobs):
    curves = []
    t50_list = []
    t90_list = []
    ...
    with mp.Pool(processes=n_jobs) as pool:
        for k, out in enumerate(pool.imap(worker_fn, arglist, chunksize=10), start=1):
            curve, t50, t90 = out
            curves.append(curve)
            t50_list.append(np.nan if t50 is None else t50)
            t90_list.append(np.nan if t90 is None else t90)
    ...
    t50_arr = np.array(t50_list, dtype=float)
    t90_arr = np.array(t90_list, dtype=float)

    return avg_curve, t50_arr, t90_arr
...
def make_results_table(t50_dynamic, t50_static, t90_dynamic, t90_static):
    rows = []

    row50 = compare_distributions(t50_dynamic, t50_static)
    row50["threshold"] = "T50"
    rows.append(row50)

    row90 = compare_distributions(t90_dynamic, t90_static)
    row90["threshold"] = "T90"
    rows.append(row90)
...
def main():
    ...
    I_dynamic_avg, T50_dynamic, T90_dynamic = run_ensemble(dynamic_worker, dyn_args, N_JOBS)
    ...
    I_static_beta1_avg, T50_static_beta1, T90_static_beta1 = run_ensemble(
        static_worker, static_args_beta1, N_JOBS
    )
    ...
    for beta_eff in BETA_GRID:
        ...
        I_static_avg, T50_static, T90_static = run_ensemble(static_worker, args, N_JOBS)

        mse = np.mean((I_static_avg - I_dynamic_avg) ** 2)
        mse_values.append(mse)
        static_results_by_beta[beta_eff] = (I_static_avg, T50_static, T90_static)
    ...
    I_static_best_avg, T50_static_best, T90_static_best = static_results_by_beta[best_beta]
    ...
    # --------------------------------------------------------
    # Compare T50 and T90 distributions
    # --------------------------------------------------------
    results_df = make_results_table(
        T50_dynamic, T50_static_best,
        T90_dynamic, T90_static_best
    )
    ...
    # --------------------------------------------------------
    # Histograms for T50 and T90
    # --------------------------------------------------------
    def finite_only(x):
        x = np.asarray(x, dtype=float)
        return x[np.isfinite(x)]

    t50d = finite_only(T50_dynamic)
    t50s = finite_only(T50_static_best)
    t90d = finite_only(T90_dynamic)
    t90s = finite_only(T90_static_best)

    plt.figure(figsize=(8, 5))
    plt.hist(t50d, bins=40, density=True, alpha=0.5, label="Dynamic T50")
    plt.hist(t50s, bins=40, density=True, alpha=0.5, label=f"Static T50, beta_eff={best_beta:.4f}")
    plt.xlabel("T50")
    plt.ylabel("Density")
    plt.title("BA network: T50 distributions")
    plt.legend()
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(8, 5))
    plt.hist(t90d, bins=40, density=True, alpha=0.5, label="Dynamic T90")
    plt.hist(t90s, bins=40, density=True, alpha=0.5, label=f"Static T90, beta_eff={best_beta:.4f}")
    plt.xlabel("T90")
    plt.ylabel("Density")
    plt.title("BA network: T90 distributions")
    plt.legend()
    plt.tight_layout()
    plt.show()
